---
entregable: mt10-innovacion-tecnologica/fidelizacion-retailco
checklist_item: "ROI: ¿por qué es necesario hacerlo? ¿por qué ahora?"
research_usado:
  - RetailCo-modelo-financiero-roi.md (incluye la "Lectura crítica" ya escrita en el propio research — este plan la sigue al pie de la letra)
  - RetailCo-arquitectura-y-costos-ia-cloud.md (CAPEX y OPEX por escenario)
  - RetailCo-sintesis-gaps-y-data-request.md (3 experimentos de des-arriesgamiento)
  - RetailCo-modelo-economico-unit-economics.md (fórmula de margen, RetailCo Card)
  - competidores.md (ventana competitiva)
  - RetailCo-dimension-negocio.md (salvedad [SV] sobre padrón de RetailCo Community)
...
regla_dura: "NO liderar con VAN $5,24M / ROI 1.505,7% — presentar el rango
  ...
  la Lectura crítica ya escrita en RetailCo-modelo-financiero-roi.md."
---
...
`RetailCo-modelo-financiero-roi.md`, este plan se aparta deliberadamente del
...
1. Depende de un escenario "base" que ya asume 18% de adopción sobre un
   padrón de RetailCo Community que es, a su vez, un supuesto **[SV]** sin
   validar (`RetailCo-dimension-negocio.md`).
...
- **35%-45% de las transacciones presenciales pierden trazabilidad de
  ...
  (`RetailCo-modelo-economico-unit-economics.md`). Cada mes de demora es un
  mes más de datos de cliente que se generan y se pierden sin capturar,
  sobre 12M transacciones/mes.
- **RetailCo es el único de los dos líderes de market share sin wallet digital
  ...
  (`competidores.md`). Mientras tanto, RetailCo depende de un tercero (Mercado
...
- **Fuga de margen recurrente a terceros**: 45% de las ventas se paga con
  ...
  de esa masa que no migra a RetailCo Card es comisión que sale del grupo económico
  cada mes, indefinidamente, con o sin proyecto.

### 1.2. Beneficio esperado

El árbol de 7 palancas del dossier (`RetailCo-modelo-financiero-roi.md`)
...
interna de RetailCo (más sólido).
...
| 3 | Migración a RetailCo Card (CU-01 nudge de medio de pago) | **Alto [EC]** | Decisión operativa interna — no requiere que el cliente compre más, solo que pague distinto |
...
| 7 | Monetización de Retail Media agéntico | **Bajo [SV]** | Terceros (proveedores pagando por espacio patrocinado) — ni siquiera depende de RetailCo o del cliente |
...
de perder margen que ya generamos" (palanca 3, migración a RetailCo Card) y "vamos a
...
- **Ventana competitiva concreta y con fecha**: Carrefour lanzó su wallet
  ...
  dos mecanismos de fidelización en paralelo. La ventana para que RetailCo
...
- **Activo propio ya construido y subutilizado**: RetailCo Card ya existe, retiene
  100% del valor dentro del grupo, y RetailCo Digital ya lidera el e-commerce
  del sector — la inversión no parte de cero, capitaliza infraestructura
  y base de clientes que RetailCo ya tiene y que la competencia no tiene en la
  misma escala (`RetailCo-dimension-negocio.md`).

## 3. El rango completo (no el número optimista)

| Variable | Conservador | Base | Optimista |
|---|---|---|---|
| Tasa de adopción (MAU/RetailCo Community) | 10,0% | 18,0% | 28,0% |
| Uplift de frecuencia de compra | +2,0% | +4,5% | +7,0% |
| Uplift de ticket promedio | +1,5% | +3,2% | +5,0% |
| Autoservicio en reclamos | 40,0% | 65,0% | 80,0% |
| WACC | 16,0% | 12,0% | 10,0% |

*(`RetailCo-modelo-financiero-roi.md`)*
...
RetailCo Community) todavía no están validados con datos internos de RetailCo.
...
depende de una cifra base ([SV], padrón de RetailCo Community) que todavía no
está confirmada por RetailCo. Se nombra este riesgo como el central del
...
(`RetailCo-sintesis-gaps-y-data-request.md`), presentados como **fase 0** del
...
2. **Piloto de promociones conversacionales en WhatsApp**: mide tasa de
   ...
   Medio [PX], proxy de empresas comparables) por un dato propio de RetailCo
...
- **Control de canibalización de canales**: verificar que el crecimiento
  de RetailCo Digital no sea solo migración de compras que el cliente ya hacía
...
`RetailCo-arquitectura-y-costos-ia-cloud.md`) permite escalonar la inversión:
...
| Retorno a 36 meses, escenario base y su peor caso de sensibilidad | — | VAN entre $2,15M y $5,24M USD; ROI entre ~620% y ~1.506% | **Adopción** — depende de un padrón de RetailCo Community [SV] sin validar |
...
el research (`RetailCo-modelo-financiero-roi.md`) solo definió los *supuestos*
