...
research_usado:
  - RetailCo-procesos-y-casos-de-uso-agenticos.md
  ...
  - RetailCo-modelo-financiero-roi.md (palanca #4 del árbol de valor)
---
...
proceso #6 del inventario (`RetailCo-procesos-y-casos-de-uso-agenticos.md`) —
...
(`RetailCo-modelo-financiero-roi.md`), justamente porque depende de una
decisión operativa interna de RetailCo, no de que el cliente cambie de
comportamiento.
...
sección 5). Se arma esa matriz con lo que el research confirma sobre RetailCo
...
| | **Humano** | **No Humano** |
|---|---|---|
| **Presencial** | Personal de sucursal, cajeros (`RetailCo-ecosistema-actores.md`: "Operaciones/Cajas") | **No confirmado** — no hay evidencia pública de autoservicio/kiosco en sucursal en el research disponible |
| **Telefónico** | 0800-888-4848 (RetailCo hotline) — confirmado como canal de derivación desde redes sociales (`RetailCo-redes-sociales-omnicanalidad.md`) | **No confirmado** — no hay evidencia pública de IVR o bot de voz |
| **Digital** | **No confirmado** — no hay evidencia de atención humana por DM/chat en RetailCo Digital o redes | **Débil/ausente** — WhatsApp es solo "Channels" de difusión unidireccional (no chatbot conversacional); no se encontró evidencia de asistente conversacional en RetailCo Digital ni en redes sociales |
...
consistente con lo que ya señala `RetailCo-redes-sociales-omnicanalidad.md`:
...
- **Qué problema resuelve**: proceso actual de resolución de reclamos
  ...
  6% de tasa de resolución diferida. *(`RetailCo-procesos-y-casos-de-uso-agenticos.md`,
  proceso #6)*
...
- **Valor esperado**: reduce el costo por ticket de soporte de $3,50 a
  ...
  autoservicio esperado en el escenario base (`RetailCo-modelo-financiero-roi.md`).
...
### 3.2. CU-06 — Asistente Conversacional RetailCo Card
...
- **Qué problema resuelve**: consultas de disponible, refinanciación de
  ...
  #7 (autenticación y cobro en RetailCo Card, 500K resúmenes/mes, 3-5 min y $0,90 USD
  cada uno).
- **Nivel de autonomía**: Nivel 2 (Recomendación) — **informa y recuerda,
  ...
  regulatoria BCRA (`RetailCo-sintesis-gaps-y-data-request.md`) — este plan
...
- **Qué dato necesita**: core bancario de RetailCo Card (saldo, historial, fechas de
  vencimiento) — uno de los 5 puntos de integración técnica con mayor
  incertidumbre señalados en `RetailCo-sintesis-gaps-y-data-request.md`, por lo
...
- **PCI DSS**: CU-06 no debe procesar ni almacenar PAN/CVV directamente —
  tokenización estándar sobre el core bancario de RetailCo Card
  (`RetailCo-datos-gobernanza-regulatorio.md`).
- **Normas BCRA para PSP**: aplican a cualquier integración con RetailCo Card,
...
- **Ley 24.240**: los guardrails de CU-04 deben evitar que el agente
  prometa una reposición o reembolso que la política real de RetailCo no
...
8). Si el equipo de RetailCo hotline sigue siendo medido por volumen de llamadas
