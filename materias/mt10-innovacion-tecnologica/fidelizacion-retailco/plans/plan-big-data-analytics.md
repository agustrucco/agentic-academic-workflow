...
research_usado:
  - RetailCo-arquitectura-y-costos-ia-cloud.md
  - RetailCo-datos-gobernanza-regulatorio.md
  - RetailCo-sintesis-gaps-y-data-request.md
  - RetailCo-modelo-economico-unit-economics.md
  - RetailCo-ecosistema-actores.md
  - marco-teorico-omnicanalidad-crm-analytics.md (8 principios McKinsey, sección 8)
  - RetailCo-modelo-financiero-roi.md (plan de medición de incrementalidad, reutilizado como "loop" del principio 5)
---
...
**Problema concreto**: los datos de RetailCo están fragmentados en sistemas que
no se integran entre sí. El mapa de actores del dossier marca
explícitamente a "Sistemas / Data RetailCo" con la fricción de **"sistemas
legacy heredados, dificultad de integración en tiempo real"**
(`RetailCo-ecosistema-actores.md`) — el POS presencial, RetailCo Digital, el core
bancario de RetailCo Card, el WMS de stock y el CRM de RetailCo Community operan hoy como
...
punto por punto al caso RetailCo — no como checklist genérico, sino mostrando
dónde este proyecto específico cumple o corre riesgo de fallar cada
principio.

## 1. Los 8 principios aplicados a RetailCo

### Principio 1 — Hacer las preguntas correctas

La pregunta de negocio de este proyecto **no es** "¿qué patrones muestra
el Big Data de RetailCo?" — es concreta y medible: **"¿cómo recuperamos
...
(`RetailCo-modelo-economico-unit-economics.md`). Cada componente de IA de este
...
Escala**, `RetailCo-arquitectura-y-costos-ia-cloud.md`) son la aplicación
...
de costo (`RetailCo-sintesis-gaps-y-data-request.md`) — y recién escalar a
...
cliente" no transaccional que hoy no se aprovecha en absoluto en RetailCo. Se
...
referencia (`RetailCo-arquitectura-y-costos-ia-cloud.md`) une en un mismo
orquestador de agentes datos que hoy están separados — POS presencial,
navegación de RetailCo Digital, core bancario de RetailCo Card, WMS de stock, y (con la
...
`RetailCo-sintesis-gaps-y-data-request.md`) son loops previos al lanzamiento
...
`RetailCo-arquitectura-y-costos-ia-cloud.md`), no como costo accesorio sino
...
La estructura de equipo del CAPEX (`RetailCo-arquitectura-y-costos-ia-cloud.md`)
...
- **CU-04/CU-06 en atención al cliente** (`plan-atencion-cliente.md`): si
  el equipo de RetailCo hotline sigue siendo medido por volumen de llamadas y no
...
|---|---|---|---|
| Perfil del cliente | CRM RetailCo Community | Alta (PII) | Ley 25.326 |
...
| Medios de pago y RetailCo Card | Core bancario / RetailCo Card | **Crítica (PCI)** | PCI DSS, normas BCRA |
| Stock por sucursal | WMS / Inventarios | Interna | Promesa de entrega sin stock real |

*(`RetailCo-datos-gobernanza-regulatorio.md`)* — esta tabla es la misma que
...
dossier (`RetailCo-sintesis-gaps-y-data-request.md`):

1. Middleware de terminales POS en cajas (latencia <800ms requerida por
   CU-01).
2. Core bancario de RetailCo Card (autenticación, disponible, historial —
   requerido por CU-06).
3. WMS y terminales de pickers en RetailCo Digital (requerido por CU-03).
4. Base de CRM/padrón de RetailCo Community (sincronización bidireccional).
...
(`RetailCo-arquitectura-y-costos-ia-cloud.md`): hit rate de caché de contexto
...
POS. Ninguna de las tres está validada con datos internos de RetailCo — son
