...
research_usado:
  ...
  - RetailCo-modelo-financiero-roi.md (diseño RFM del plan de incrementalidad)
...
(`RetailCo-modelo-financiero-roi.md`: "filtrado por segmento RFM sobre la base
de socios RetailCo Community"). Este plan adopta RFM como la capa de
...
un cliente use RetailCo Card o consigne su DNI, mejora la calidad del propio dato de
...
- **Qué dato necesita**: contexto de la transacción en curso, perfil del
  ...
  de pago habilitados para ese cliente (si tiene RetailCo Card).
- **Doble función de negocio**: además de optimizar la transacción
  ...
  la migración hacia RetailCo Card — la palanca de mayor confianza del árbol de valor
...
- **Usuario**: cliente de RetailCo Digital/App.
...
- **Qué dato necesita**: historial de compra del cliente (para inferir
  ...
  física" en el inventario de datos, `RetailCo-datos-gobernanza-regulatorio.md`
...
- **Middleware de integración POS** — señalado como una de las 5
  ...
  (`RetailCo-sintesis-gaps-y-data-request.md`): la latencia POS-Cloud real no
...
- **PCI DSS / normas BCRA (aplica a CU-01)**: el dato "medios de pago
  habilitados para ese cliente (si tiene RetailCo Card)" que necesita CU-01 sale del
  Core bancario/RetailCo Card, marcado "Crítica (PCI)" en
  `RetailCo-datos-gobernanza-regulatorio.md`. El agente no debe procesar ni
  almacenar PAN/CVV — solo un flag tokenizado de elegibilidad RetailCo Card, siguiendo
