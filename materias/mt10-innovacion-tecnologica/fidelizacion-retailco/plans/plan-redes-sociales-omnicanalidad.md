...
research_usado:
  - RetailCo-redes-sociales-omnicanalidad.md (research principal — el dossier de negocio no cubre este tema)
  ...
  - RetailCo-procesos-y-casos-de-uso-agenticos.md (CU-01, CU-04 — reutilizados como interfaz adicional, no como agentes nuevos)
...
**Problema concreto**: las redes sociales de RetailCo están desconectadas del
resto del ecosistema de datos y de atención. Con la evidencia pública
disponible, RetailCo se ubica en **"Multichannel"** (varios canales activos en
...
RetailCo/RetailCo Card, y los reclamos iniciados en redes (confirmado en X,
`RetailCo-redes-sociales-omnicanalidad.md`) se derivan sistemáticamente al 0800
...
## 1. Diagnóstico honesto: dónde está RetailCo en la escala de madurez omnicanal
...
evidencia pública disponible, el diagnóstico de RetailCo en lo que hace a
redes sociales es:

- **Presencia**: fuerte y multiplataforma — Instagram (@coto_ar, ~808K-810K
  seguidores), Facebook (@RetailCo, ~1,1M "me gusta"), X (@Coto_Ar), TikTok
...
- **Lo que falta para Crosschannel/Omnichannel**: no hay evidencia pública
  ...
  el perfil de RetailCo Community/RetailCo Card, ni de que un reclamo iniciado en redes
...
- Aplicando el framework de 3 pilares (Recognition/Engagement/Orchestration,
  Winterberry Group): RetailCo tiene **Engagement** (contenido, presencia,
  volumen de audiencia) pero no hay evidencia de **Recognition** (reconocer
  al mismo cliente en redes y en RetailCo Community/RetailCo Card) ni de **Orchestration**
  ...
  exclusiva de RetailCo: solo 8,9% de +100 marketers encuestados logra
...
`RetailCo-redes-sociales-omnicanalidad.md` marca este hallazgo como "basado en
ausencia de evidencia pública, no como hecho confirmado de que no existe
integración" — es posible que RetailCo tenga integración interna (CRM, tracking
...
## 2. Qué rol cumplen hoy las redes sociales en RetailCo (confirmado)
...
reclamos en el mismo hilo/DM. Este patrón no es exclusivo de RetailCo: Carrefour
...
### 3.2. Componente 1 — Cerrar el loop de datos entre redes y RetailCo Community (fundacional, sin IA)

- **Qué se propone**: vincular la interacción social (clic desde bio/post,
  ingreso vía canal de WhatsApp) al perfil de RetailCo Community mediante
  tracking de UTM y, opcionalmente, un login social en el alta de Comunidad
  RetailCo (sección 2.1 de `plan-plataforma-demanda-campanas.md`).
...
- **Marca explícita de propuesta especulativa**: no hay research que
  confirme si esta integración ya existe internamente en RetailCo. Se
...
- **Qué problema resuelve**: hoy el cliente que interactúa por WhatsApp
  ...
  hoy si pago con RetailCo Card?" ni iniciar un reclamo — tiene que cambiar de canal.
...
`RetailCo-sintesis-gaps-y-data-request.md`, extendido a este tema):

1. Confirmar si existe o no integración interna entre redes sociales y el
   CRM de RetailCo Community (el gap más importante señalado por el propio
   research).
