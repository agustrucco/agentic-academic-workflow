...
framing: "Plataforma agéntica" — ver decisión de framing en
  .claude/agent-memory/tp-solution-architect/mt10-fidelizacion-retailco-framing.md
---
...
de baja relevancia. El proceso #8 del inventario (`RetailCo-procesos-y-casos-
...
(tráfico, awareness) no es el cuello de botella — RetailCo Digital ya lidera el
e-commerce del sector (`RetailCo-dimension-negocio.md`) — el cuello de botella
...
cupones de **3% a 12%** (`RetailCo-procesos-y-casos-de-uso-agenticos.md`) — sin
...
demanda de RetailCo sobre ese ciclo, no como tres módulos separados sino como
tres momentos del mismo dato de cliente:

- **Machear**: identificar al cliente correcto en el momento correcto (alta
  de RetailCo Community, reconocimiento en punto de venta/app).
...
`RetailCo-modelo-economico-unit-economics.md`), cualquier campaña —por
...
- **Canal de entrada**: alta de miembro RetailCo Community — hoy 40K
  solicitudes/mes, con **8,5% de tasa de error por datos inválidos** en el
  proceso actual (3-5 min, $1,20 USD por alta). *(`RetailCo-procesos-y-casos-de-uso-agenticos.md`,
  proceso #1)*
...
- **Canales de demanda existentes que alimentan este componente**: RetailCo
  Digital (líder de e-commerce del sector), redes sociales (awareness —
  desarrollado en `plan-redes-sociales-omnicanalidad.md`), RetailCo Card como
...
demanda en sí (tráfico a RetailCo Digital, awareness de marca) es
mayoritariamente un problema de marketing tradicional (medios pagos,
presencia en redes, beneficio ancla de RetailCo Community) que ya funciona —
RetailCo ya lidera el sector en e-commerce. El componente de IA que sí agrega
...
~50 campañas/mes. *(`RetailCo-procesos-y-casos-de-uso-agenticos.md`)*
...
| Reglas de promociones vigentes y su financiamiento (RetailCo/proveedor/banco) | Engine de promociones | Confirmado — 3 estructuras financieras distintas (`RetailCo-modelo-economico-unit-economics.md`) |

**Punto crítico de diseño**: el agente tiene que distinguir explícitamente
qué promociones son 100% RetailCo (erosionan margen propio), cofinanciadas con
...
- **Ley 24.240 (Defensa del Consumidor)**: el agente no puede alucinar una
  ...
  de cualquier envío (`RetailCo-datos-gobernanza-regulatorio.md`).
- **Ley 25.326 (Protección de Datos Personales)**: el perfilamiento
  ...
  explícito del socio de RetailCo Community — este consentimiento debe
