# Guion de presentación — Plataforma de fidelización RetailCo

**Formato confirmado** (`consignas/mt10-fidelizacion-retailco.md`): 10 minutos,
...
contexto puro (participación de mercado, tamaño de RetailCo) se dice en una
...
**Diapositiva 1 — Título**: "Plataforma de fidelización RetailCo" + nombres del
equipo.
...
Guion hablado:
- RetailCo lidera el mercado de supermercados (22,3%) y lidera, sin discusión,
  el e-commerce del sector con RetailCo Digital. No venimos a proponer que RetailCo
  entre al retail digital — ya lo lidera.
- El problema es otro: entre 35% y 45% de las transacciones presenciales se
  cierran sin que el cliente consigne su DNI o su carnet de RetailCo Community.
...
**Diapositiva 3 — Tabla comparativa reducida**: solo 3 filas (RetailCo,
...
Guion hablado:
- De los dos líderes de participación de mercado, RetailCo es el único sin
...
- La ventana para que RetailCo entre a esta categoría sin quedar en tercer
...
1. **En caja**: en menos de 800 milisegundos, el sistema sugiere la
   ...
   RetailCo Card, además de mejorar la transacción puntual, mejora la calidad del
...
4. **En RetailCo Digital**: el cliente arma su lista de compras dentro de un
...
RetailCo Digital, core RetailCo Card, stock, redes) convergiendo en "un orquestador de
...
- Retoma el bloque 2 en una frase: la ventana frente a Carrefour y Cencosud
  se angosta cada trimestre, y el activo para capturarla —la RetailCo Card—
  ya existe.
...
- El riesgo principal no es tecnológico ni de costo de infraestructura de
  ...
  **adopción**, y depende de una cifra que todavía no está validada por la
  compañía: el tamaño real del padrón de RetailCo Community.
...
- Por eso no pedimos autorizar el CAPEX completo hoy. Pedimos autorizar una
  ...
  los supuestos más importantes de este caso por evidencia propia de RetailCo.
...
- Arrancamos con los dos casos de uso de mayor confianza y menor
  ...
  campañas, e-commerce y el asistente de la RetailCo Card.
- Esto no es solo tecnología. Tres cambios de gestión tienen que pasar
  ...
  RetailCo hotline (de volumen de llamadas a calidad de resolución), redefinir el
...
- **Con transparencia**: todavía no tenemos diseñado el detalle de quién es
  ...
  RetailCo, ni el cronograma exacto dentro de cada fase. Es el siguiente paso
  de trabajo si el directorio da luz verde a la fase 0, y necesita
  información interna de la estructura organizacional de RetailCo que hoy no
  tenemos.

### Bloque 7 — Cierre (9:15–10:00) · **Voz 1** (cierra quien abrió)

Guion hablado:
- RetailCo no necesita entrar al retail digital: ya lo lidera. Lo que
...
- No les pedimos que confíen en un número de retorno extraordinario. Les
  ...
  —empezando por el tamaño real del padrón de RetailCo Community— por evidencia
  propia de RetailCo.
...
1. *"¿De dónde sale el padrón de 3,5 a 6,5 millones de socios de Comunidad
   RetailCo?"* → Es un supuesto pendiente de validación con información interna
   ...
   primer insumo que necesitamos de RetailCo antes de comprometer el
...
5. *"¿Por qué RetailCo Card y no seguir con Mercado Pago?"* → Mercado Pago es
   un tercero: cada pago digital vía MP no deja dato propio ni retiene
   margen financiero dentro del grupo. La RetailCo Card ya existe, retiene
   100% del valor dentro de RetailCo, y es la base natural para construir
...
8. *"¿Quién es responsable puertas adentro de que el personal de sucursal y
   de RetailCo hotline realmente adopten esto?"* → Es un hueco que el documento
...
este contexto. No hay un color de marca de RetailCo ya definido en este
repositorio para reemplazarlo — si el equipo quiere usar el verde/rojo de
identidad de RetailCo, es una decisión a tomar aparte, no algo que este guion
